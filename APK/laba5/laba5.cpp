...
void menu();                   //меню
void setTime();                //установка время
void showTime();               //вывод время
void setAlarm();               //установка будильника
void enterTime();              //ввод нового время
void resetAlarm();             //сброс будильника
int convertToBCD(int);         //перевод в бцд код
int convertToDecimal(int);     //10
...
void interrupt newTime(...) // новый обработчик прерываний часов
{
    ...
    outp(0x20, 0x20);   //master
    outp(0xA0, 0x20);//slave
}
...
int convertToDecimal(int BCD) //в 10 сс
{
    return ((BCD / 16 * 10) + (BCD % 16));
}

int convertToBCD(int decimal)// в бцд код
{
